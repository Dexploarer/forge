  ...
  fastify.post('/generate-image', {
    ...
    try {
      ...
      if (env.AI_GATEWAY_API_KEY || env.VERCEL_ENV) {
        ...
        // These are multimodal text models that can generate images
        // Use generateText() and extract images from result.files
        // Build the image generation prompt
        const imagePrompt = `Generate an image with the following specifications:
- Description: ${prompt}
- Size: ${size}
- Quality: ${quality}
- Style: ${style}

Please create a high-quality image that matches this description.`
        ...
        fastify.log.info(`[Image Generation] Result received - hasFiles: ${!!result.files}, fileCount: ${result.files?.length || 0}`)
        ...
        // The text response may contain information about the image
